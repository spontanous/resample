...
Fs_ref=44e3;		% 1.1) Reference Sampling frequency
...
eta_min_dB=-28;	% 4.2) Lower limit constraint
...
w_bins_smooth=5;
b_smooth=hanning(2*w_bins_smooth+1);
b_smooth=b_smooth/sum(b_smooth);     % normalize the window function
 bin_500Hz=round(500/Fs*M+1);
 w_bins_smooth=w_bins_smooth+2;
for l=1:Nframes
    if l==1
    ...
    if (~fnz_flag && abs(y(1))>zero_thres) ||  (fnz_flag && any(abs(y)>zero_thres))       % new version omlsa3
        ...
        temp=conv(G,b_smooth);
        G_smooth=temp(w_bins_smooth+1:M21+w_bins_smooth);
        G(bin_500Hz:end,1)=G_smooth(bin_500Hz:end,1);
...
%         X=[zeros(2,1); G(3:M21-1).*Y(3:M21-1); 0];
    ...
    if l==1
        ...
        fwrite(foutID,[out(1:Mno)*2^15],'int16');
    else
